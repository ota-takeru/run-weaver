...
daemonは通常、`run-weaver repo add` で登録したrepository一覧を `repos.json` から読みます。継続pollではpollごとに `repos.json` を読み直すため、登録追加後もdaemon再起動なしで次回pollから対象になります。repoごとに `gh --repo <owner/repo>`、repo専用clone、repo専用state fileを使い、repo内では既存どおり1 jobずつ、repo間では同時実行します。`--repo-url` と `--repo` を指定した場合は互換用の単一repoモードとして動きます。`--once` は登録済みrepositoryを1回ずつ確認し、各repoで最大1件開始します。
