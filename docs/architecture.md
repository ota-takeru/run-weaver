...
Plannerとtask promptはドキュメント衝突を2種類に分けます。`docs/progress.md`、`docs/handoff.md`、`docs/process-log.md` のような現在状態・引き継ぎ・時系列ログは運用ドキュメントとして扱い、それらを複数taskが触るだけでは依存関係やstacked PRを作りません。task-localな進捗、検証、handoffはGitHub Issueコメント、PR本文、run-weaver stateを優先し、複数taskのあとに必要なら最後のドキュメント統合taskで現在状態を整えます。README、`docs/architecture.md`、`docs/cli.md`、`docs/github-issue-flow.md`、`docs/decision-log.md`、`docs/adr/`、migration、lockfile、公開API、共有service/testは意味的な変更として扱い、同じ判断や挙動を複数taskが変更する場合はdependency、stacked PR、またはdecision gateの材料にします。

Dispatcherはローカルstate fileの `campaign` を正本にして、依存関係が解決済みの次taskを選びます。taskごとに既存のclaim、worktree、tmux、Codex、draft PR flowを再利用します。Campaign taskは同じworktree上で `plan`、`implement`、`review`、`verify` の順にCodexを起動し、`verify` 完了後だけcommit、push、draft PR作成へ進みます。Campaign taskは既定でcompletion orderのstacked PRにし、2件目以降のtaskは直前に完了したCampaign taskのbranchをworktree baseとdraft PR baseに使います。Plannerの `dependencies[]` は実行可能順序とdecision gate用の意味的依存を表し、機械的なPR stackingはDispatcherが保証します。task開始時またはphase進行時にworktree、Doppler、prompt、runnerで失敗した場合は、子Issue、Campaign task、Campaign status、state jobを `blocked` に揃えます。task完了後はPR URLをCampaign stateへ記録し、次taskへ進めます。

Decision Requestには `decision`、`context`、`evidence`、`options`、`option details`、`recommendation`、`impact`、`reversibility`、`blocked tasks`、`can continue tasks` を含めます。観測済み事実、推論、推奨を分け、secret値やJSONLログ本文は載せません。人間はPCでは `run-weaver decision answer --repo <owner/repo> '#<campaign-issue>' <decision-id> <option>` で回答し、外出先ではDecision Request内のquick replyから `run-weaver-decision:<decision-id>:<option>` の1行を選んでGitHub Issueへ新規コメントします。CLIとdaemonはいずれもローカルstateまたはDecision Requestの定義済みoptionに照らして回答を検証します。pending decisionがある間は `can continue tasks` に含まれるtaskだけを実行し、実行可能taskがないdecision gateではCampaignを `decision_required` として停止し、回答を読み取ったらstateへ保存して再開します。回答済みdecisionの内容は後続Campaign task promptへ含めます。task dependencyは `depends: task-...` のtask ID形式を正とします。
