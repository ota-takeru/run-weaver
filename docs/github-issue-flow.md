...
ドキュメント衝突は種類で扱いを分けます。`docs/progress.md`、`docs/handoff.md`、`docs/process-log.md` のような運用中の現在状態・引き継ぎ・時系列ログだけを複数Issueが触る可能性がある場合、それだけではstacked PRにしません。task-localな進捗や検証結果はGitHub Issueコメント、PR本文、run-weaver stateを優先し、必要なら後続のドキュメント統合taskでまとめます。README、architecture、CLI仕様、decision log、ADR、migration、lockfile、公開API、共有service/testなど、意味的に同じ挙動や判断を変更するファイルの重複は依存関係やstacked PRの判断材料にします。
