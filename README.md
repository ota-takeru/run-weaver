...
2. 人間が対象Issueに `run-weaver:ready` ラベルを付ける。Campaignの場合は `run-weaver:campaign` も付ける
...
GitHub Releasesから取得するため、ローカルにこのプロジェクトをcloneしておく必要はありません。`install` は常駐設定を作成しますが、事前に `git`、`gh`、`codex`、`doppler` の導入と認証、対象Issueの `run-weaver:ready` ラベルが必要です。Campaign Issueを使う場合は親Issueに `run-weaver:campaign` も付けます。対象repository内で実行すると、Codex専用clone用のrepository URLは `git remote get-url origin` から自動推定されます。secret値はコマンドやドキュメントに書かず、Doppler service token `dev-agent` は環境変数で渡します。
