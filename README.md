...
`install` は常駐設定を作成しますが、事前に `git`、`gh`、`codex`、`doppler` の導入と認証、Codex専用clone用のrepository URL、対象Issueの `run-weaver:ready` ラベルが必要です。secret値はコマンドやドキュメントに書かず、Doppler service token `dev-agent` は環境変数で渡します。

WSL:

```sh
go build -o run-weaver ./cmd/run-weaver
./run-weaver doctor --target wsl
./run-weaver install --target wsl
./run-weaver daemon --target wsl --repo-url https://github.com/example/repo.git --repo example/repo
```

Windows PowerShell:

```powershell
go build -o .\run-weaver.exe .\cmd\run-weaver
.\run-weaver.exe doctor --target windows
.\run-weaver.exe install --target windows --repo-url https://github.com/example/repo.git --repo example/repo
.\run-weaver.exe status --repo example/repo
```
